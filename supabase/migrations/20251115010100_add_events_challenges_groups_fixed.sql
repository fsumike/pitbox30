/*
  # Add Events, Challenges, Groups, and Verified Badges (Fixed)
...
DROP POLICY IF EXISTS "Anyone can view RSVPs for public events" ON event_rsvps;
...
-- RLS Policies for Challenge Entries
...
DROP POLICY IF EXISTS "Users can submit challenge entries" ON challenge_entries;
CREATE POLICY "Users can submit challenge entries"
  ON challenge_entries FOR INSERT
  TO authenticated
  WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can delete their own entries" ON challenge_entries;
...
DROP POLICY IF EXISTS "Anyone can view votes" ON challenge_votes;
CREATE POLICY "Anyone can view votes"
  ON challenge_votes FOR SELECT
  TO authenticated
  USING (true);

DROP POLICY IF EXISTS "Users can vote on entries" ON challenge_votes;
CREATE POLICY "Users can vote on entries"
  ON challenge_votes FOR INSERT
  TO authenticated
  WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can remove their votes" ON challenge_votes;
...
DROP TRIGGER IF EXISTS update_event_rsvp_count_trigger ON event_rsvps;
...
DROP TRIGGER IF EXISTS update_challenge_entry_count_trigger ON challenge_entries;
...
DROP TRIGGER IF EXISTS add_creator_to_group_trigger ON groups;
